...
from analysis.extraction.schema import (
    PageArtifact,
    is_acronym,
    find_table_description,
    find_image_description,
    is_widow_func,
    is_bekart_func,
    is_szewc_func,
)
...
class PDFMapper:
    ...
    def map_to_schema(old_doc: DocumentData) -> FinalDocument:
            ...
            for block in page.text_blocks:
                ...
                for line in block.lines:
                    ...
                    if line_type == "list" and full_text.strip():
                        ...
                        if prev_type == "paragraph":
                            ...
                            PDFMapper.empty_paragraph_buffer(
                                new_doc.logical_blocks,
                                paragraph_buffer,
                                "odcięcie wstępu od listy",
                            )
                            full_text = ""
                            words_info = []
                            curr_line = 0
                    ...
                    if not is_new_paragraph:
                        if not full_text.strip() and (
                            line_x0 > x0_margin + margin_indent_thresh
                        ):
                            is_new_paragraph = True
                    ...
                    if is_new_paragraph:
                        ...
                        if list_buffer:
                            PDFMapper.empty_list_buffer(
                                new_doc.logical_blocks, list_buffer
                            )
                        if paragraph_buffer:
                            PDFMapper.empty_paragraph_buffer(
                                new_doc.logical_blocks, paragraph_buffer, debug_reason
                            )
                            curr_line = 0
                ...
                if current_type:
                    PDFMapper.empty_paragraph_buffer(
                        new_doc.logical_blocks,
                        paragraph_buffer,
                        "wykryto blok opisu tabeli/zdjęcia",
                    )
                    curr_line = 0
                    PDFMapper.empty_list_buffer(new_doc.logical_blocks, list_buffer)

                    new_doc.logical_blocks.append(
                        ParagraphBlock(
                            block_id=block.block_id,
                            content=full_text,
                            words=words_info,
                ...
                if PDFMapper.is_header(words_info):
                    PDFMapper.empty_paragraph_buffer(
                        new_doc.logical_blocks, paragraph_buffer, "wykryto nagłówek"
                    )
                    curr_line = 0
                    PDFMapper.empty_list_buffer(new_doc.logical_blocks, list_buffer)
                ...
                if block_type == "list":
                    PDFMapper.empty_paragraph_buffer(
                        new_doc.logical_blocks, paragraph_buffer, "wykryto listę"
                    )
                    curr_line = 0
        ...
        PDFMapper.empty_list_buffer(new_doc.logical_blocks, list_buffer)
        PDFMapper.empty_paragraph_buffer(
            new_doc.logical_blocks, paragraph_buffer, "finalne opróżnienie"
        )
        ...
        for page in old_doc.pages:
            for table in page.tables:
                desc_text, found_side = find_table_description(
                    list(table.bbox), new_doc.logical_blocks, priority_side="above"
                )
                ...
                if final_caption:
                    for lb in new_doc.logical_blocks:
                        if (
                            hasattr(lb, "content")
                            and lb.content.strip() == final_caption
                        ):
                            lb.type = "table_description"
                            break

            for img in page.images:
                desc_text, found_side = find_image_description(
                    list(img.bbox), new_doc.logical_blocks, priority_side="below"
                )
                if desc_text:
                    for lb in new_doc.logical_blocks:
