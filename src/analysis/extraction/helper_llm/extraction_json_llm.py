...
debug_path = (
    "pdf_diploma_checker/src/redaction/redaction_debug/{debug_type}_example.pdf"
)
...
def extractPDF_llm(file_path: str) -> DocumentData:
    ...
    for page_index, page in enumerate(doc):
            ...
            if block["type"] == 1:
                ...
                if phys_width < 40 and phys_height < 40:
                    ...
                    for d in drawings:
                        d_rect = fitz.Rect(d["rect"])
                        if (
                            d_rect.width < p_width * 0.9
                            and d_rect.height < p_height * 0.9
                        ):
                ...
                description, found_side = find_image_description(
                    block["bbox"], cur_page.text_blocks, detected_img_priority
                )
        ...
        detected_img_priority = extract_vector_graphics(
            page, drawings, page_index, table_bboxes, cur_page, detected_img_priority
        )
    ...
    for raw_line in raw_block["lines"]:
        ...
        for raw_span in raw_line["spans"]:
            ...
            for x in block_words:
                ...
                if (
                    x[0] >= s_bbox[0] - 0.2
                    and x[1] >= s_bbox[1] - 0.2
                    and x[2] <= s_bbox[2] + 0.2
                    and x[3] <= s_bbox[3] + 0.2
                ):
                    span_words.append(x)
...
def is_footer(
    raw_block: dict, page_height: float, page_num: int, threshold: float = 0.88
) -> bool:
    bbox = raw_block["bbox"]
