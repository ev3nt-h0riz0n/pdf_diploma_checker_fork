    ...
    try:
        ...
        if config_path:
            ...
            validator = RedactionValidator(
                doc_data, doc_data_linguistics, str(config_path)
            )
        else:
            print(
                "Ostrzeżenie: Nie znaleziono pliku configuration.json! Walidacja uruchomiona bez reguł formatowania."
            )
        ...
        with open(output_report_path, "w", encoding="utf-8") as file:
            ...
            if not found_errors:
                ...
                print(
                    f"Pełny raport tekstowy został wygenerowany w: {output_report_path}"
                )
