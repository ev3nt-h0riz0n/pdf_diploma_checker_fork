...
def mark_bboxes_on_pdf(
    pdf_path: str, saving_path: str, json_path: str, structures: List[str] | None = None
    ...
    def draw_rect_on_page(
        page, bbox: List[float], color: Tuple[float, float, float], width: float = 1.0
    ) -> None:
        try:
            r = to_rect(bbox)
    ...
    def record_found(
        name: str, page_num: int, bbox: List[float], color: Tuple[float, float, float]
    ) -> None:
...
def mark_bboxes_from_list(
    pdf_path: str,
    saving_path: str,
    items: List[Dict[str, Any]],
    structures: List[str] | None = None,
    page_key_names: List[str] | None = None,
) -> Dict[str, List[Dict[str, Any]]]:
    ...
    def record(
        name: str, page: int, bbox: List[float], color: Tuple[float, float, float]
    ) -> None:
        lst = found.setdefault(name, [])
        lst.append(
            {
